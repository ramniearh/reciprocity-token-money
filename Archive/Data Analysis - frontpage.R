...
df_large_pop_with_means %>% 
  ...
  geom_boxplot() +
#  facet_wrap(~step) +
  ggtitle("Five strategies, large populations: Cooperation rates for different benefit/cost ratios at selected time steps")
...
summary_df_large_pop %>% 
  ...
  facet_wrap(~ bc_ratio, labeller = label_both) +
  ggtitle("Five strategies, large populations: evolution of surviving strategies by memory/forgiv (pop=1250, liquidity=1)")
