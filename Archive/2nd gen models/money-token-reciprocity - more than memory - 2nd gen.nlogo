...
to set-me-up ;REVIEWED VERSION WITH NON-STRATEGIC-SPECIFIC VARIABLES
  set fitness 0
    ...
    set my-balance one-of [ 1 1 ]; personal index, Bigoni 2020 (REVIEW OVERSUPPLY)
    shape-me
    size-me
    color-me
end
...
to go
  ...
  ask turtles [ set fitness fitness - avg-welfare ] ; normalize fitnesses:
...
to interact ; unlike BigoniCC, turtles interact twice per round, each time initiated by one member of the pair
  ...
  if ( my-strategy = "RLM" ) [ ; Bigoni MMM 2020 - supplementary material p. 10. Reputation balances supposed to be less successful than tokens (!= model results)
    ifelse ( not leaky? and [my-balance] of one-of link-neighbors > 0 ) or ( leaky? ) [ ;CONFIRM dynamics: our RLM implementation (not "leaky" only transacts with counterparts that have positive balances; Bigoni-like implementation ("leaky") places no such lower bound
      groom
...
false
false
"set-plot-x-range ( min [fitness] of turtles - 1 ) ( max [fitness] of turtles + 1 )\nset-histogram-num-bars 19" ""
PENS
"default" 1.0 0 -16777216 true "" "histogram [fitness] of turtles"
