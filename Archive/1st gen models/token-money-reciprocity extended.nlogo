...
;nowak2005  All eight of these strategies distinguish justified from unjustified defections. This property must hold for any strategy that maintains cooperation, eliminates cheats and can overcome errors49. However, the problem with the concept of justified defection is that it requires information not only about the past of the co-player but also about the past of the co-player's co-players, and their co-players, and so on36,40.
...
turtles-own [ has-token? my-strategy my-fitness my-blacklist my-balance my-reputation ]
...
to set-me-up
    ...
    if my-strategy = "grudger" [ set my-blacklist [] ]
...
to color-me
  ...
  if my-strategy = "reputation" [ set color white ]
end
...
to go
  ...
  set sucker-welfare sum [my-fitness] of turtles with [ my-strategy = "sucker" ]
  set cheater-welfare sum [my-fitness] of turtles with [ my-strategy = "cheater" ]
  set grudger-welfare sum [my-fitness] of turtles with [ my-strategy = "grudger" ]
  set token-welfare sum [my-fitness] of turtles with [ my-strategy = "token" ]
  set RLM-welfare sum [my-fitness] of turtles with [ my-strategy = "RLM" ]

  set total-welfare sum [my-fitness] of turtles
...
to interact ; unlike BigoniCC, turtles interact twice per round, each time initiated by one member of the pair
    ...
    ifelse not member? one-of link-neighbors my-blacklist [ groom ] [ not-groom ] ; netlogo peculiarity? grudger memory seems to not operate without specifying "one-of" link-neighbors
    if length my-blacklist > grudger-memory [
      set my-blacklist remove-item ( length my-blacklist - 1 ) my-blacklist ; cap length of memory string by killing last/oldest item
    ]
  ]
...
to groom
  set my-fitness my-fitness - cost
  ask link-neighbors [ ; confirm peculiar behavior "one-of" x "item 0"
    set my-fitness my-fitness + benefit
  ]
end

to not-groom
  ask link-neighbors with [ my-strategy = "grudger" ] [
    if not member? myself my-blacklist [
      set my-blacklist fput myself my-blacklist
    ]
  ]
end
...
to evolve-all
  ...
  ask min-n-of n turtles [ my-fitness ] [ die ]
  create-turtles n [ right random 180 fd random 13
    set my-fitness avg-welfare ; new turtles are born with average fitness. alternatively, reset and recalculate fitness?
    set-me-up
  ]
...
175
291
Direct reciprocity, repeated PD:\nTrivers 1971 (plus memory cap)
10
0.0
1
...
197
422
Indirect reciprocity (downstream):\nNowak 2005
10
0.0
1
...
go</go>
  ...
  <experiment name="token-grudger-cheater" repetitions="20" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
