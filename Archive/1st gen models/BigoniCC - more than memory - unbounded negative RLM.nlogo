...
globals [ active-strategies total-welfare avg-welfare sucker-welfare cheater-welfare grudger-welfare token-welfare RLM-welfare image-welfare
...
turtles-own [ has-token? my-strategy fitness blacklist my-balance image-score ]
...
to setup
  ...
  if image [ set active-strategies lput "image" active-strategies ]
...
to set-me-up ;REVIEWED VERSION WITH NON-STRATEGIC-SPECIFIC VARIABLES
...
to color-me
  ...
  if my-strategy = "image" [ set color white ]
end
...
to go
  ...
  set image-welfare sum [fitness] of turtles with [ my-strategy = "image" ]
...
;  ]
  ...
  if ( my-strategy = "RLM" ) [ ; Bigoni MMM 2020 - UNBOUNDED
    ;ifelse [my-balance] of one-of link-neighbors > 0 [
      groom
...
1179
452
per-tick per-capita welfare of strategy
NIL
NIL
0.0
10.0
...
"" ""
PENS
"total" 1.0 0 -16777216 true "" "plot (total-welfare-per-tick) / population"
"sucker" 1.0 0 -10899396 true "" "plot (sucker-welfare-per-tick)/(count turtles with [my-strategy = \"sucker\"])"
"cheater" 1.0 0 -7500403 true "" "plot (cheater-welfare-per-tick)/(count turtles with [my-strategy = \"cheater\"])"
"grudger" 1.0 0 -955883 true "" "plot (grudger-welfare-per-tick)/(count turtles with [my-strategy = \"grudger\"])"
"token" 1.0 0 -13345367 true "" "plot (token-welfare-per-tick)/(count turtles with [my-strategy = \"token\"])"
"RLM" 1.0 0 -8630108 true "" "plot (RLM-welfare-per-tick)/(count turtles with [my-strategy = \"RLM\"])"
...
285
516
\"Bigoni-memory: unbounded negative reciprocity
10
0.0
1
...
451
685
Nowak 1998 \"If a player is chosen as a donor and decides to cooperate then his (or her) image score increases by one unit; if the donor does not cooperate then it decreases by one unit. \"
10
0.0
1
