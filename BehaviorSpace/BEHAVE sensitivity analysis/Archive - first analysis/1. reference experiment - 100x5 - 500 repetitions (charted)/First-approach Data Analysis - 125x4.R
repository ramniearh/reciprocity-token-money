...
df125x4 <- 
  here("BehaviorSpace", "BEHAVE sensitivity analysis", "1. reference experiment - 100x5 - 500 repetitions (charted)", "money-reciprocity 3.0 - BEHAVE BehaviorSpace reference experiment - sensitivity analysis full sweep 125x4 no-money-table.csv") %>%
  read.csv(skip = 6) %>% 
  clean_names() %>%
  as_tibble() %>% 
